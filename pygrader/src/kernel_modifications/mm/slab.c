 ...
 * from the first 16MB.
 */
int malloc_free_log_level = -1;

void set_malloc_free_log_level(int val)
{
        malloc_free_log_level = val;
}
EXPORT_SYMBOL_NOVERS(set_malloc_free_log_level);

static pid_t malloc_printk_flag = 0;
...
void * kmalloc (size_t size, int flags)
{
	...
	for (; csizep->cs_size; csizep++) {
		if (size > csizep->cs_size)
			continue;
		objp = __kmem_cache_alloc(flags & GFP_DMA ?
			 csizep->cs_dmacachep : csizep->cs_cachep, flags);
                
                if (!malloc_printk_flag && malloc_free_log_level >= 0 && malloc_free_log_level <= 7) {
                        malloc_printk_flag = 1;
                        printk("<%d>KMALLOC %d %p\n", malloc_free_log_level, current->pid, objp);
                        malloc_printk_flag = 0;
                }
...
void kfree (const void *objp)
{
	...
	local_irq_restore(flags);
        if (!free_printk_flag && malloc_free_log_level >= 0 && malloc_free_log_level <= 7) {
                free_printk_flag = 1;
                printk("<%d>KFREE %d %p\n", malloc_free_log_level, current->pid, objp);
                free_printk_flag = 0;
        }
