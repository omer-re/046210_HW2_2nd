//
// Created by omer.reuveni on 6/14/2021.
//
...
void scheduler_tick(int user_tick, int system)
{
    ...
    // TODO: flow control
    /**
     * if current.is_privileged ==1: check for older procs, which aren't sleeping(!)
     * if there is-> do set_reshedule,
     * else-> do "out"
     *
     * if current.is_privileged ==0:
     * check if there are waiting privileged procs.
...
////// TODO: replace idx = sched_find_first_bit(array->bitmap);
//     check for older, active processes on priority_queue
//     if p.q isn't empty- check for the oldest non sleeping (max jiffies) proc, return it to next
//      if p.q is empty - use regular sched by checking     idx = sched_find_first_bit(array->bitmap);
#if CONFIG_SMP
    pick_next_task:
#endif
